...
def import_steps(dir_file)
  ...
  Dir.glob(File.join(dir_file, "**/*.pdf")) do |file|
	...
	puts("	"+"Performing Requested OCR Process...(Importing #{file})") 
...
def identify_pdf_word_count_average()
	...
	# over 100 is kept and not tagged
...
def perform_ocr_detection()
	...
	puts ("Performing Requested OCR Process...(ID Must OCR Documents)") 
	identify_must_ocr() if IDENTIFY_MUST_OCR_FLAG
	puts("	"+"Performing Requested OCR Process...(ID 500KB+ Images)")
	identify_images_over_500kb() if IDENTIFY_IMAGES_OVER_500KB_FLAG
	puts("	"+"Performing Requested OCR Process...(ID 1MB+ Images)")
	identify_images_over_1mb() if IDENTIFY_IMAGES_OVER_1MB_FLAG
	puts("	"+"Performing Requested OCR Process...(ID 5MB+ Images)")
	identify_images_over_5mb() if IDENTIFY_IMAGES_OVER_5MB_FLAG
	puts("	"+"Performing Requested OCR Process...(Calculating Word Count Averages)")
