...
begin
	...
	userScriptsLocation=userScriptsLocation
...
begin
	...
	analyser = $utilities.getNlpAnalyser()
	#puts analyser

	analyser.setCredentials($creds['NLPSAAS'])
	analyser.testConnection()

	map1 = {'includeFamilies'=>config.customArguments['includeFamilies'], 'updateDuplicates'=>config.customArguments['updateDuplicates']}
	nlpJob = analyser.createJob(items.to_a, map1)
	lastState=nlpJob.getStatus()['state']
	checkState="-1"
	while(!(["COMPLETED","RETRIEVED"].include? lastState)) # COMPLETED should be the only state required to check for but if you run two NLP jobs on the same case... one will eventually fail and stay in RETRIEVED state.
		if(checkState != lastState)
			puts Time.new.to_s + " State Changed to #{lastState}."
	...
	puts "finished with final state of #{lastState}"
